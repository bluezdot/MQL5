...
input bool   InpCloseAtSessionEnd    = false;       // Enable closing positions before session ends
input int    InpCloseBeforeMinutes   = 60;          // [InpCloseAtSessionEnd=true] Minutes before session end to close all positions
...
//+------------------------------------------------------------------+
//| Close positions X minutes before session end (per-tick check)    |
//+------------------------------------------------------------------+
void CheckSessionEndClose() {
   if (g_sessionCloseDone)          return;
   if (g_range.dayMidnight == 0)    return;  // session not yet initialized

   // Session end = session start + 24h
   datetime sessionEnd  = g_range.dayMidnight + 86400;
...
void CheckDayReset(datetime barTime) {
   ...
   g_range.dayMidnight    = sessionStart;
   g_range.high           = 0.0;
   g_range.low            = DBL_MAX;
...
void RunStateMachine() {
   ...
   if (g_state == ORB_IDLE) {
      // The initial range candle is the bar whose index from session-start equals InpOpenBar
      if (barIndexOfDay == InpOpenBar) {
         ...
         if (InpMinComposition == 0) {
            g_state = ORB_FINAL;
            Print("[ORB] Single-bar range FINALIZED (bar #", InpOpenBar, "). High=", g_range.high, " Low=", g_range.low);
         } else {
            g_state = ORB_FORMING;
