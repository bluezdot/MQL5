...
#property description "Time Range Breakout (TRB) Expert Advisor - Custom time range breakout strategy"
//+------------------------------------------------------------------+
//| Input parameters                                                 |
//+------------------------------------------------------------------+
input group "==== General Inputs ===="
input double InpLotSize = 0.01;
input long InpMagicNumber = 20;

input group "==== SL / TP Inputs ===="
input int InpSLPct = 100; // stop loss in % of the range (0=off)
input int InpTPPct = 100; // take profit in % of the range (0=off)

input group "==== Time Range Inputs ===="
input int InpRangeStartHour   = 22;  // Range start hour in UTC (0–23, e.g. 22 = 22:00 UTC)
input int InpRangeStartMinute = 0;   // Range start minute (0–59)
input int InpRangeEndHour     = 9;   // Range end hour in UTC (0–23, e.g. 4 = 04:00 UTC next day)
input int InpRangeEndMinute   = 0;   // Range end minute (0–59)
input int InpRangeCloseHour   = 20;  // Range close hour in UTC (0–23). Set -1 to disable automatic close
input int InpRangeCloseMinute = 0;   // Range close minute (0–59). Ignored if InpRangeCloseHour = -1
...
int OnInit() {
  ...
  if (InpRangeStartHour < 0 || InpRangeStartHour > 23) {
    Alert("InpRangeStartHour must be between 0 and 23");
    return INIT_FAILED;
  }

  if (InpRangeStartMinute < 0 || InpRangeStartMinute > 59) {
    Alert("InpRangeStartMinute must be between 0 and 59");
    return INIT_FAILED;
  }

  if (InpRangeEndHour < 0 || InpRangeEndHour > 23) {
    Alert("InpRangeEndHour must be between 0 and 23");
    return INIT_FAILED;
  }

  if (InpRangeEndMinute < 0 || InpRangeEndMinute > 59) {
    Alert("InpRangeEndMinute must be between 0 and 59");
    return INIT_FAILED;
  }

  int _startMins = InpRangeStartHour * 60 + InpRangeStartMinute;
  int _endMins   = InpRangeEndHour   * 60 + InpRangeEndMinute;

  if (_startMins == _endMins) {
    Alert("Range start and end times must be different");
    return INIT_FAILED;
  }

  if (InpRangeCloseHour < -1 || InpRangeCloseHour > 23) {
    Alert("InpRangeCloseHour must be -1 (disabled) or between 0 and 23");
    return INIT_FAILED;
  }

  if (InpRangeCloseHour >= 0 && (InpRangeCloseMinute < 0 || InpRangeCloseMinute > 59)) {
    Alert("InpRangeCloseMinute must be between 0 and 59");
    return INIT_FAILED;
  }
...
void OnTick() {
  ...
  Comment("Range Start: ", range.start_time, "\n",
   ...
   if (InpRangeCloseHour >= 0 && lastTick.time >= range.close_time && range.close_time > 0) {
      if(!ClosePositions()) {return;}
   }
   
   // time range calculation
   if (((InpRangeCloseHour >= 0 && lastTick.time >= range.close_time)                      // close time reached
   ...
   // detect new bar and check for breakouts only when a bar closes
   datetime currentBar = iTime(_Symbol, _Period, 0);
   if (currentBar != g_lastBar) {
      g_lastBar = currentBar;
      
      // Get the close price of the bar that JUST finished
...
void CalculateRange () {
   ...
   // Compute start in minutes and auto-derive duration (supports cross-midnight sessions)
   int      startMins    = InpRangeStartHour * 60 + InpRangeStartMinute;
   int      endMins      = InpRangeEndHour   * 60 + InpRangeEndMinute;
   int      durationMins = (endMins - startMins + 1440) % 1440;  // e.g. 22:00→04:00 = 360 min

   // Session-aware start time: shift axis, find day boundary, shift back.
   // Correctly handles sessions starting before UTC midnight.
   datetime startOffset  = (datetime)(startMins * 60);
   datetime adjustedTime = lastTick.time - startOffset;
   datetime adjustedDay  = adjustedTime - (adjustedTime % time_cycle);
   range.start_time = adjustedDay + startOffset;
   for (int i=0; i<8; i++) {
      MqlDateTime tmp;
      TimeToStruct(range.start_time, tmp);
      int dow = tmp.day_of_week;
      if (lastTick.time >= range.start_time + durationMins*60 || dow == 6 || dow == 0 || (dow == 1 && !InpMonday) || (dow == 2 && !InpTuesday) || (dow == 3 && !InpWednesday) || (dow == 4 && !InpThursday) || (dow == 5 && !InpFriday)) {
         range.start_time += time_cycle;
      }
   }

   // calculate range end time
   range.end_time = range.start_time + durationMins*60;
   for (int i=0; i<2; i++) {
      MqlDateTime tmp;
   ...
   // calculate range close (InpRangeCloseHour = -1 means disabled)
   if (InpRangeCloseHour >= 0) {
      int closeMins = InpRangeCloseHour * 60 + InpRangeCloseMinute;
      range.close_time = (range.end_time - (range.end_time % time_cycle)) + (datetime)(closeMins * 60);
      for (int i=0; i<3; i++) {
         MqlDateTime tmp;
...
void CheckBreakouts(double closePrice) {
   // check if we are after the range end
   if (lastTick.time >= range.end_time && range.end_time > 0 && range.f_entry) {
      // check for high breakout (only if bar closed above range.high)
      if (InpBreakoutMode != BREAKOUT_MODE_LOW && !range.f_high_breakout && closePrice > range.high) {
         ...
         // cal sl & tp
         double sl = InpSLPct == 0 ? 0 : NormalizeDouble(lastTick.bid - ((range.high - range.low) * InpSLPct * 0.01), _Digits);
         double tp = InpTPPct == 0 ? 0 : NormalizeDouble(lastTick.bid + ((range.high - range.low) * InpTPPct * 0.01), _Digits);

         trade.PositionOpen(_Symbol, ORDER_TYPE_BUY, InpLotSize, lastTick.ask, sl, tp, "Range Breakout Buy (Close)");
      }
   
      // check for low breakout (only if bar closed below range.low)
      if (InpBreakoutMode != BREAKOUT_MODE_HIGH && !range.f_low_breakout && closePrice < range.low) {
         range.f_low_breakout = true;
         ...
         // cal sl & tp
         double sl = InpSLPct == 0 ? 0 : NormalizeDouble(lastTick.ask + (range.high - range.low) * InpSLPct * 0.01, _Digits);
         double tp = InpTPPct == 0 ? 0 : NormalizeDouble(lastTick.ask - (range.high - range.low) * InpTPPct * 0.01, _Digits);

         trade.PositionOpen(_Symbol, ORDER_TYPE_SELL, InpLotSize, lastTick.bid, sl, tp, "Range Breakout Sell (Close)");
      }
   }
}
...
void DrawObjects() {
   ...
   if (range.high > 0) {
      ...
      ObjectCreate(NULL, "range high forward", OBJ_TREND, 0, range.end_time, range.high, InpRangeCloseHour >= 0 ? range.close_time : INT_MAX, range.high);
   ...
   if (range.low < DBL_MAX) {
      ...
      ObjectCreate(NULL, "range low forward", OBJ_TREND, 0, range.end_time, range.low, InpRangeCloseHour >= 0 ? range.close_time : INT_MAX, range.low);
