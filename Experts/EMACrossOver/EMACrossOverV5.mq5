...
//--- create EMA handles
   ...
   if(ema_short_handle == INVALID_HANDLE || ema_long_handle == INVALID_HANDLE || ema_longterm_handle == INVALID_HANDLE)
     {
...
void OnTick() {
  //--- get EMA values
   double ema_short[1], ema_long[1], ema_longterm[1];
  ...
  //--- check for buy signal
   if(ema_short[0] > ema_long[0] && currentPrice > ema_longterm[0] && PositionsTotal() == 0)
     {
      trade.Buy(LotSize, _Symbol, 0, 0, 0, "Open EMA Crossover Buy (Price > EMA200)");
     }
