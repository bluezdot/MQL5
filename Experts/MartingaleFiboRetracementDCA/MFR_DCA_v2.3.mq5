...
input double   InpTpRetracementPct    = 0.3;    // TP Retracement % from avg entry toward anchor (0.0-1.0)
//+------------------------------------------------------------------+
//| Include files                                                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Cập nhật Take Profit theo Average Entry + Buffer                 |
//| TP = AverageEntry ± (anchor - AverageEntry) × TP_RETRACEMENT_PCT |
//|   - Đảm bảo có lãi khi hit TP                                    |
//|   - TP gần hơn anchor → Tăng tỉ lệ hit                           |
//+------------------------------------------------------------------+
...
void UpdateTP(int dir) {
    ...
    // 1. Tính average entry price (lot-weighted)
    for(int i = PositionsTotal()-1; i >= 0; i--) {
        ulong ticket = PositionGetTicket(i);
        if(PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber) {
            if((dir ==  1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ||
               (dir == -1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)) {
                ...
                totalLot        += lot;
                totalValue      += lot * openPrice;
                count++;
            }
        }
    }
    if(count == 0 || totalLot == 0) return;

    double avgEntry = totalValue / totalLot;
    TrendState chain = (dir == 1) ? buyChain : sellChain;

    // 2. Tính TP = Average Entry + buffer về phía anchor
    double tpBuffer  = (chain.anchor - avgEntry) * InpTpRetracementPct;
    double currentTP = avgEntry + tpBuffer;  // Buy: anchor > avg → buffer dương
                         // Sell: anchor < avg → buffer âm

    // 3. Apply TP cho tất cả positions cùng TP
    for(int i = PositionsTotal()-1; i >= 0; i--) {
        ulong ticket = PositionGetTicket(i);
        if(PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber) {
            if((dir ==  1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ||
