...
#property description "Martingale Fibonacci Retracement DCA Expert Advisor - v2.01"
//+------------------------------------------------------------------+
//| Input parameters                                                 |
//+------------------------------------------------------------------+
input double   InpBaseLot             = 0.01;   // Base Lot Size
input double   InpLotMultiplier       = 1.0;    // Lot Multipler
...
input long     InpMagicNumber         = 15;     // Magic Number
//+------------------------------------------------------------------+
//| Include files                                                    |
//+------------------------------------------------------------------+
...
struct TrendState {
    bool   active;
    double anchor;       // Điểm gốc (f0): Buy = SwingHigh (đỉnh trend); Sell = SwingLow (đáy trend)
    double limit;        // Điểm giới hạn (f1): Buy = SwingLow (đáy pullback); Sell = SwingHigh (đỉnh pullback)
        ...
        if(PositionSelectByTicket(ticket) && PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == InpMagicNumber) {
            if((dir ==  1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ||
               (dir == -1 && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL))
                trade.PositionClose(ticket); // TODO: Handle case close position failed
        }
    }
}
...
//+------------------------------------------------------------------+
// TODO: Sử dụng iLowest, iLow thay vì while
...
void SetupDCAChain(int dir, double anchor, double limitPt) {
    ...
    if(dir == 1) {
        ...
        for(int i = 0; i < 10; i++) {
            ...
            if(!trade.BuyLimit(InpBaseLot * InpLotMultiplier * fiboMult[i], price, _Symbol, 0, 0))
        ...
        for(int i = 0; i < 10; i++) {
            ...
            if(!trade.SellLimit(InpBaseLot * InpLotMultiplier * fiboMult[i], price, _Symbol, 0, 0))
...
//|   Buy:  bidPrice >= anchor (vượt đỉnh → pullback không xảy ra)   |
//|   Sell: bidPrice <= anchor (rơi dưới đáy → pullback sai)         |
//+------------------------------------------------------------------+
void CheckAndManageChain(TrendState &state) {
    ...
    if(!hasPos) {
        ...
        if((state.direction ==  1 && askPrice >= state.anchor) ||
           (state.direction == -1 && bidPrice <= state.anchor)) {
            ResetChain(state);
            return;
        }
...
bool IsNewBar() {
    static datetime lastBarTime = 0; // set first time
